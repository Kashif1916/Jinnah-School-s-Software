<?php
/**
 * Drop & Restore Student - Core Module
...
$can_drop = is_master() || is_admission();

// Determine current view mode
$default_view = $can_drop ? 'drop_mode' : 'see_mode';
$view_mode = $_GET['view'] ?? $default_view;

// Restrict Finance & Teacher from accessing drop_mode or restore_mode
if (!$can_drop && $view_mode !== 'see_mode') {
    $view_mode = 'see_mode';
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'bulk_drop') {
    if (!$can_drop) {
        $error = 'You do not have permission to drop students.';
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'bulk_restore') {
    if (!is_master()) {
        $error = 'Only Master admin can restore dropped students.';
    } else {
        $student_ids = $_POST['student_ids'] ?? [];
...
if ($view_mode === 'drop_mode' && $can_drop) {
...
if ($view_mode === 'restore_mode' && is_master()) {
...
<body>
    <div class="wrapper feature-shell">
        <main class="main-content">
            <?php render_role_topbar_and_nav('Drop & Restore Student', 'drop_student'); ?>
            ...
            <div class="form-section">
                ...
                <?php if ($view_mode === 'drop_mode' && $can_drop): ?>
                ...
                <?php elseif ($view_mode === 'see_mode'): ?>
                    ...
                    <div class="dropped-section mt-4">
                        ...
                        <?php else: ?>
                            <div class="alert alert-info">No dropped students found with these filters!</div>
                        <?php endif; ?>
                    </div>

                <!-- ==================== MODE 3: WANT TO RESTORE DROP STUDENT ==================== -->
                <?php elseif ($view_mode === 'restore_mode' && is_master()): ?>
                    <div class="search-section mb-4">
