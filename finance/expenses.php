...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'add_expense') {
        ...
        if ($expense_id <= 0 || $amount <= 0 || empty($reason)) {
            ...
            // Strict Security Check: Sirf wahi expense edit hoga jo LOGGED-IN USER ka ho AUR AJ (CURDATE) ki date ka ho!
            $check_stmt = $conn->prepare("SELECT id FROM expenses WHERE id = ? AND user_id = ? AND DATE(created_at) = CURDATE()");
            $check_stmt->bind_param("ii", $expense_id, $my_user_id);
            $check_stmt->execute();
